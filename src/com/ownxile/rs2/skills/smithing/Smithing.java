...
public class Smithing {
	...
	public Smithing(Client c) {
		this.c = c;
		superHeat = new SuperHeat(c);
	}

	private SuperHeat superHeat = null;
		...
		if (c.getItems().playerHasItem(toremove, toremove2)) {
			...
			if (maketimes > 1
				...
				c.sendMessage("You make some "
						+ ItemAssistant.getItemName(toadd) + ".");
			} else {
			...
			while (maketimes > 0) {
				if (c.getItems().playerHasItem(toremove, toremove2)) {
					...
					} else if (ItemAssistant.getItemName(toadd)
							.contains("nail")) {
						c.getItems().addItem(toadd, 15);
					} else if (ItemAssistant.getItemName(toadd).contains(
							"arrow")) {
						c.getItems().addItem(toadd, 15);
					} else if (ItemAssistant.getItemName(toadd).contains(
							"knife")) {
					...
					c.getFunction().addSkillXP(
							xp * SkillConfig.SMITHING_EXPERIENCE, 13);
					c.getFunction().refreshSkill(13);
					maketimes--;
				} else {
					break;
				}
	...
	public void readInput(int level, String type, Client c, int amounttomake) {
		...
		if (ItemAssistant.getItemName(Integer.parseInt(type))
		...
		} else if (ItemAssistant.getItemName(Integer.parseInt(type)).contains(
				"Iron")) {
			...
			CheckMith(c, level, amounttomake, type);
				...
				"Adam")
				|| ItemAssistant.getItemName(Integer.parseInt(type)).contains(
						"Addy")) {
	...
	public void sendSmelting() {
		for (int j = 0; j < SMELT_FRAME.length; j++) {
			c.getFunction().sendFrame246(SMELT_FRAME[j], 150, SMELT_BARS[j]);
		}
		c.getFunction().sendFrame164(2400);
	...
	public void smelt(int barType) {
		if (c.smeltAmount > 0) {
			...
			if (hasOres(barType)) {
				...
				c.getFunction()
						.addSkillXP(exp * SkillConfig.SMITHING_EXPERIENCE,
								c.playerSmithing);
				c.getFunction().refreshSkill(c.playerSmithing);
				c.smeltAmount--;
				c.smeltTimer = 1;
				c.startAnimation(894);
				c.sendMessage("You smelt one of the ores into a bar.");
			} else {
	...
	/**
	 * @param superHeat
	 *            the superHeat to set
	 */
	public void setSuperHeat(SuperHeat superHeat) {
		this.superHeat = superHeat;
	}

	/**
	 * @return the superHeat
	 */
