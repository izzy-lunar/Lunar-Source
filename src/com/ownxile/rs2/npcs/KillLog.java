...
public class KillLog {
	...
	public static void killInterface(Client player) {
		player.getFunction().sendFrame126("@blu@NPC Kill Log", 8144);

		player.getFunction().sendFrame126("", 8145);
		player.getFunction().sendFrame126("", 8146);
		for (int i = 0; i < 101; i++) {
			player.getFunction().sendFrame126("", 8147 + i);
		}
		for (int i = 0; i < NPCS.length; i++) {
			if (NPCS[i] > 0) {
				String name = World.getNpcHandler().getNPCName(NPCS[i]);
				player.getFunction().sendFrame126(
						"@dre@" + name + "@bla@: " + player.loggedKills[i],
						8147 + i);
			}
		}
		player.getFunction().showInterface(8134);
	}
	...
	public static void handleKill(Client player, int npcType) {
		if (isNpc(npcType)) {
			...
			String name = World.getNpcHandler().getNPCName(npcType);
			...
			if (shouldNotify(player.loggedKills[index])) {
				World.sendMessage("@grd@" + player.playerName
