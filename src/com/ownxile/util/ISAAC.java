...
public class ISAAC {
	...
	private static final int SIZEF = 8; // Log of size of rsl[] and mem[]
	private static final int SIZEG = 1 << SIZEF; // Size of rsl[] and mem[]
	private static final int SMASK = SIZEG - 1 << 2; // For pseudorandom lookup
	...
	private final void init(boolean flag) {
		...
		for (i = 0; i < SIZEG; i += 8) { // Fill in mem[] with messy stuff
			if (flag) {
				a += rsl[i];
				b += rsl[i + 1];
				c += rsl[i + 2];
	...
	private final void isaac() {
		...
		for (i = 0; i < SIZEG; ++i) {
			...
			a += mem[i + SIZEG / 2 & SIZEG - 1];
			mem[i] = y = mem[(x & SMASK) >> 2] + a + b;
			rsl[i] = b = mem[(y >> SIZEF & SMASK) >> 2] + x;
		}
	}
